...
很多网友在评论区提问：“Termux 有办法打个镜像或者快照吗？怕折腾坏了”。
其实，**备份就是最好的快照**。

在 Termux-X 中进行备份，本质上就是对当前的容器环境（文件系统）进行一次完整的打包。
当你遇到以下情况时，备份将是你最强的“后悔药”：

1.  **无惧折腾**：尝试安装新软件、修改系统配置导致环境崩溃时，可以一键还原。
2.  **误删后悔**：手滑执行了 `rm -rf` 删除了关键文件，备份能救你一命。
3.  **环境迁移**：换了新手机，或者想把配置好的环境分享给朋友，备份包就是最好的载体。
4.  **快照存档**：配置好一个完美的开发环境后，打个包存起来，随时可以回滚。

养成定期备份的好习惯，可以让你在探索 Linux 世界时无后顾之忧。
...
可在左侧边栏的`工具箱 > 备份/恢复`中找到.

![backup-restore](/img/common/9.png)

### 备份

![backup-restore](/img/common/10.png)
